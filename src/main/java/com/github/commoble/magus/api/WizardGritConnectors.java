...
public class WizardGritConnectors
{
	...
	/**
	 * Add blocks to the registry *after* all blocks have been registered! FMLCommonSetupEvent is a good time to do it
	 * If a provider was previously registered, returns it, otherwise returns null 
	 **/
	@Nullable
	...
	/**
	 * Convenience method to register a block that is its own connection provider (not required)
	 * If a provider was previously registered, returns it, otherwise returns null 
	 **/
	@Nullable
	...
	/** Gets a provider from the registry, returning (()->empty set) if no provider is registered to the given block **/
	@Nonnull
	...
	/** Use a specific block for the connection provider instead of the one at the given position
	 * -- useful for when a block is *about* to be placed **/
	@Nonnull
	...
	/** Returns the set of all positions that A) the given position connects to, and B) connect to the given position **/
	@Nonnull
	...
	/** Returns true if the blocks at the given positions are registered connectors and can connect to each other, false otherwise **/
	...
	/**
	 * data class for storing a source block and its known potential connections, whose purposes are
	 * A) prevent recalculation more often than necessary, and
	 * B) to ensure that a connection set has been verified to be a given blockpos's potential connections
	 * for best results, do not store instances of this in fields; only use as method arguments
	 *
	 */
	public static class PartialConnection
	{
		...
		public boolean isMutuallyConnectedTo(@Nonnull IBlockReader world, @Nonnull BlockPos targetPos)
		{
			return this.getPotentialConnections().contains(targetPos) &&
				WizardGritConnectors.doesPotentialConnectionExist(world, targetPos, this.getSourcePos());
		}
	}
}
